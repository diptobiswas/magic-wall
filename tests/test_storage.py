...
def test_state_api_drops_legacy_dashboard_data(tmp_path: Path) -> None:
    cfg = AppConfig(config_path=tmp_path / "config.toml", data_dir=tmp_path / "data")
    storage = WallStorage(cfg)
    storage.write_state(
        {
    ...
    assert "dashboard" not in api_state
    assert api_state["story"]["title"] == "A story"
